...
from config import HF_API_KEY  # Fixed typo in import

API_URL = "https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-inpainting"
HEADERS = {"Authorization": f"Bearer {HF_API_KEY}"}  # Fixed f-string syntax
...
def main():
    ...
    prompt = input("Describe the restoration (or type 'exit'): ")
    ...
    try:
        ...
        fixed.show()
        if input("Save restored image? (yes/no): ").lower() == "yes":
