...
def summarize_text(text, min_length, "max_length": max length)
payload = {
    "inputs": text,
...
if __name__ == "__name__":
    pritnt(Fore.YELLOW + style.BRIGHT + "???? Hi There! What's your name?")
    ...
    if not user_text:
        print(Fore.RED + "❌No text provided. Eciting.")
    else:
    ...
    if style_choice == "2":
        ...
        summary = summarize_text(user_text, min_lenght, max_lenght, model_name=model_choice)
        if summary:
